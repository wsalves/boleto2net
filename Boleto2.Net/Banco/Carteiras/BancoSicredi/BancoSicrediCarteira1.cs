...
namespace Boleto2Net
{
    ...
    internal class BancoSicrediCarteira1 : ICarteira<BancoSicredi>
    {
        ...
        {
            if (boleto.Banco.Cedente.ContaBancaria.OperacaoConta.Length != 2)
                throw new NotImplementedException($"Não foi possível formatar o campo livre: Codigo de Operação da conta ({boleto.Banco.Cedente.ContaBancaria.OperacaoConta}) não possui 2 dígitos.");

            if (boleto.Banco.Cedente.Codigo.Length != 5)
                throw new NotImplementedException($"Não foi possível formatar o campo livre: Conta ({boleto.Banco.Cedente.Codigo}) não possui 5 dígitos.");

            if (boleto.Banco.Cedente.ContaBancaria.Agencia.Length != 4)
                throw new NotImplementedException($"Não foi possível formatar o campo livre: Numero de Agencia ({boleto.Banco.Cedente.ContaBancaria.Agencia}) não possui 4 dígitos.");
